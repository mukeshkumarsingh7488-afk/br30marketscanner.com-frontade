...
const MULTI_ASSET_MARKETS = ["forex-majors", "forex-cross", "metals", "metal-stocks", "commodities", "us-stocks", "us-etfs"];
...
const canOpenChart = (market = "") => !isOptionMarket(market) && !isComingSoonMarket(market);
...
const formatLtp = (value, market) => {
  ...
  if (["metals", "metal-stocks", "commodities", "us-stocks", "us-etfs"].includes(market)) {
...
const getTradeCall = (row = {}, market = "") => {
  market = normalizeMarket(market || row.market);
  ...
  if (signal.includes("watch buy") || signal.includes("watch sell")) return "WAIT";
  ...
  if (signal === "buy") {
    if (isStockMoveMarket(market)) return move >= 2 && volume > 0 ? "BUY" : "WAIT";
    return "BUY";
  }

  if (signal === "sell") {
    if (isStockMoveMarket(market)) return move <= -2 && volume > 0 ? "SELL" : "WAIT";
    return "SELL";
  }
...
export default function ScannerTable({ rows = [], market = "future-stock", lastUpdated = "" }) {
  ...
  const isComingSoon = isComingSoonMarket(normalizedMarket);
  ...
  const openChart = (row) => {
    if (isOptionMarket(normalizedMarket) || isOptionMarket(row.market) || isComingSoon) return;
    ...
    <section className="tableBox">
      <div className="tableHead tableHeadPro">
        <div>
          ...
          <p>{isComingSoon ? "This scanner is coming soon." : "Live scanner data with signal, trade call and chart access where available."}</p>
        </div>

        <div className="tableSearchBox">
          <span className="tableLastUpdated">⏱ Last Updated: {lastUpdated || "--"}</span>

          <input type="text" value={search} placeholder="Search symbol..." onChange={(e) => setSearch(e.target.value)} disabled={isComingSoon} />
      ...
      {isComingSoon ? (
        <div className="empty" style={{ padding: "30px", textAlign: "center" }}>
          🚧 Coming Soon
        </div>
      ) : (
            ...
            <tbody>
              {filteredRows.length === 0 ? (
                ...
                filteredRows.map((s, i) => {
                  const call = getTradeCall(s, normalizedMarket);
